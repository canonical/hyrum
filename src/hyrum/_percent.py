...
A single helper so both keep the same floor: a non-zero count never renders
as ``0%``, because a table cell reading ``0%`` beside a count of ``1`` looks
...
def format_pct(fraction: float, *, decimals: int = 0) -> str:
    ...
    precision, ``<0.1%`` at one decimal place.
    """
    pct = fraction * 100
