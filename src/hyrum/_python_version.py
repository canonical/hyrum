...
def wrap_with_uv_python(
    ...
    *,
    ...
    ``with_packages`` names the packages providing ``cmd``, and is what makes
    the wrap do anything: ``uv run --python`` substitutes the requested
    interpreter only for ``python``/``python3`` itself. Any other command is
    resolved from ``PATH`` and, being a console script with a fixed shebang,
    keeps running under whatever interpreter installed it — uv creates no
    ephemeral environment and does not touch ``PATH`` unless ``--with`` asks
    it to. With ``--with tox``, uv installs tox for the requested interpreter
    and puts it first on ``PATH``, which is what changes the interpreter tox
    then picks as its default ``basepython``.

    A ``cmd`` that is not resolved from ``PATH`` (an absolute path, or an
    executable given as ``python -m tox``) is unaffected by ``--with``, so
    the wrap remains a no-op for those.
