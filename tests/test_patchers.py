...
def test_min_python_from_pyproject_absent():
    assert _python_version.min_python_from_pyproject({}) is None


def test_wrap_with_uv_python_noop_without_version():
    assert _python_version.wrap_with_uv_python(('poetry', 'lock'), None, ('uv',)) == (
        'poetry',
        'lock',
    )
...
def test_wrap_with_uv_python_prefixes_uv_run():
    assert _python_version.wrap_with_uv_python(('poetry', 'lock'), (3, 12), ('uv',)) == (
        'uv',
        'run',
        '--no-project',
        '--python',
...
def test_wrap_with_uv_python_respects_uv_executable():
    assert _python_version.wrap_with_uv_python(
